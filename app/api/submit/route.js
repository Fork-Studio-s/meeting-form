...
const REQUIRED_FIELDS = ['name', 'email', 'phone', 'business', 'handle', 'role', 'service', 'curiosity', 'outcome', 'timeline', 'budget'];

// Default floor pulled from Gabeln's documented local project minimum (IDR 20-25mio).
// Tune here as real budget data comes in, or split by service if needed.
const LOW_BUDGET_VALUE = 'Under IDR 20mio';
const LONG_TIMELINE_VALUES = ['6 months – 1 year', '> 1 year'];

function scoreLead(data) {
  const lowBudget = data.budget === LOW_BUDGET_VALUE;
  const managerRole = data.role === 'Marketing or Brand Manager';
...
// Mirrors the freeTextTriggers config in app/page.js's STEPS array.
// If a step's trigger value changes there, update it here too, or the
// typed follow-up will silently vanish from the internal email.
...
function resolveField(data, field) {
  const trigger = FREE_TEXT_TRIGGERS[field];
  if (trigger && data[field] === trigger) {
    return data[`${field}Other`] || data[field];
  }
  return data[field];
}
...
function buildInternalEmailHtml(data, score) {
  const rows = [
    ...
    ['Role', resolveField(data, 'role')],
    ['Service', resolveField(data, 'service')],
    ['Most curious about', resolveField(data, 'curiosity')],
    ['Outcome expected', resolveField(data, 'outcome')],
  ...
  const rowsHtml = rows
    .map(
      ([label, value]) => `
    <tr>
      <td style="padding:8px 12px;color:#706a62;font-size:13px;border-bottom:1px solid #eee;">${escapeHtml(label)}</td>
      <td style="padding:8px 12px;font-size:14px;border-bottom:1px solid #eee;"><strong>${escapeHtml(value || 'not answered')}</strong></td>
    </tr>`
    )
    .join('');
  ...
  return `
    <div style="font-family:-apple-system,Helvetica,Arial,sans-serif;max-width:560px;margin:0 auto;">
      <span style="display:inline-block;background:${badgeColor(score.tag)};color:#fff;padding:6px 14px;border-radius:4px;font-size:13px;font-weight:600;">${score.tag}</span>
      ...
      <p style="color:#999;font-size:12px;margin-top:20px;">Submitted ${new Date().toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })}</p>
    </div>
  `;
}
...
function buildConfirmationEmailHtml(data) {
  ...
  const service = escapeHtml(resolveField(data, 'service') || 'Design & Development');
  const outcome = escapeHtml(resolveField(data, 'outcome') || 'growth');
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0" style="max-width:500px;background-color:#ffffff;border:1px solid #e0e0e0;border-radius:12px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,0.03);">
                ...
                <tr>
                  <td style="padding:36px 32px;">
                    ...
                    <h1 style="margin:0 0 18px;font-size:22px;font-weight:500;color:#201e1b;line-height:1.3;">Thanks, ${firstName}.</h1>
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#f7f7f7;border:1px solid #e0e0e0;border-radius:8px;margin-bottom:24px;">
                      <tr>
                        <td style="padding:16px 20px;">
                          <div style="font-size:12px;color:#919191;text-transform:uppercase;letter-spacing:0.04em;margin-bottom:4px;">Exploring</div>
                          <div style="font-size:14px;font-weight:600;color:#201e1b;margin-bottom:12px;">${service}</div>
                          <div style="font-size:12px;color:#919191;text-transform:uppercase;letter-spacing:0.04em;margin-bottom:4px;">Target Outcome</div>
...
export async function POST(request) {
  ...
  if (db) {
    try {
      await db.collection('leads').add({
        ...
        role: resolveField(data, 'role'),
        service: resolveField(data, 'service'),
        curiosity: resolveField(data, 'curiosity'),
        outcome: resolveField(data, 'outcome'),
        timeline: data.timeline,
        budget: data.budget,
        scoreTag: score.tag,
  ...
  try {
    await transporter.sendMail({
      ...
      replyTo: data.email,
      subject: `New lead: ${data.business} (${score.tag})`,
    ...
    await transporter.sendMail({
      ...
      to: data.email,
      subject: `We've got your info, ${data.name}`,
      html: buildConfirmationEmailHtml(data),
    });
  } catch (err) {
    console.error('Email send failed:', err);
    return Response.json({ error: 'Failed to send. Please try again in a moment.' }, { status: 502 });
  }
